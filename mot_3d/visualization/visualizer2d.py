...
class Visualizer2D:
    ...
    def handler_box(self, box: BBox, message: str='', color='red', linestyle='solid', label=''):
        if label == '':
            if color == 'red':
                label = 'Alive Track'
        ...
        plt.plot(corners[:, 0], corners[:, 1], color=self.COLOR_MAP[color], linestyle=linestyle, label=label)
