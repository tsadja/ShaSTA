...
class PubTracker(object):
  def __init__(self,  hungarian=False, max_age=0, refine_confidence=False,alpha = 0.5, beta=0.5):
  ...
  def step_centertrack(self, results, time_lag):
    ...
    for m in matches:
      ...
      # NEW IDEA
      if self.refine_confidence:
        prev_track_conf = self.tracks[m[1]]['ref_detection_score']
        tp_prob = track['ref_detection_score']
        det_conf = track['detection_score']
        track['ref_detection_score'] = (tp_prob > self.alpha) * self.beta * det_conf + (1-self.beta) * prev_track_conf
        #track['ref_detection_score'] = self.beta * det_conf + (tp_prob > self.alpha) * (1-self.beta) * prev_track_conf
        #track['ref_detection_score'] = self.beta * det_conf + (-1)**(tp_prob < self.alpha) * (1-self.beta) * prev_track_conf
    ...
    for i in unmatched_dets:
      ...
      '''
      if self.refine_confidence:
        track['ref_detection_score'] = self.beta*track['detection_score']
      '''
    ...
    for i in unmatched_tracks:
      ...
      if track['age'] < self.max_age:
        ...
        '''
        if self.refine_confidence:
            track['ref_detection_score'] = (1-self.beta)*track['ref_detection_score']
        '''
    ...
    '''
    all_scores = [track['ref_detection_score'] for track in ret]
    min_score, max_score = min(all_scores), max(all_scores)
    for i, trk in enumerate(ret):
        if max_score != min_score:
            ret[i]['ref_detection_score'] = (trk['ref_detection_score']-min_score)/(max_score-min_score)
        else:
            ret[i]['ref_detection_score'] /= max_score
    '''
