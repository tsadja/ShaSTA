...
import os
import numpy as np
from pub_tracker_merged import PubTrackerMerged as TrackerMerged
...
def parse_args():
    ...
    parser.add_argument("--work_dir", help="the dir to save logs and tracking results", 
                        # default = "data/nusc_detections/cp")
                        default="work_dir/results")
    parser.add_argument(
        "--checkpoint", help="the dir to checkpoint which the model read from",
        default = "work_dir/merged_results/merged_cp_val.json"
        # default = "cp-train.json"
    )
...
def main():
    ...
    tracker = TrackerMerged(max_age=args.max_age, hungarian=args.hungarian)
    ...
    for i in range(size):
        ...
        for item in outputs:
            ...
            nusc_anno = {
                ...
                "tracking_score": item['ref_detection_score'],
            }
...
def eval_tracking():
    ...
    eval(os.path.join(args.work_dir, 'tracking_result.json'),
        args.version,
        args.split,
        os.path.join(args.work_dir, 'visualize'),
        args.root
    )
