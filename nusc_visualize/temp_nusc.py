...
class TempNuScenes:
    ...
    def render_sample_data(self, sample_data_token: str, with_anns: bool = True, tracks: List[TrackingBox] = [],
                           ...
                           show_panoptic: bool = False) -> None:
        self.explorer.render_sample_data(sample_data_token, with_anns, tracks, box_vis_level, axes_limit, ax, nsweeps=nsweeps,
                                         out_path=out_path,
...
class NuScenesExplorer:
    ...
    def __init__(self, nusc: TempNuScenes):
        self.nusc = nusc
    ...
    def render_sample_data(self,
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            boxes = []
            for track in tracks:
                box = Box(center= track.translation, size=track.size, orientation= Quaternion(track.rotation), name=track.tracking_name)
                if use_flat_vehicle_coordinates:
                    # Move box to ego vehicle coord system parallel to world z plane.
                    yaw = Quaternion(pose_record['rotation']).yaw_pitch_roll[0]
                    box.translate(-np.array(pose_record['translation']))
            ...
            if with_anns:
                ...
                gt_boxes = [b for b in gt_boxes if 'car' in b.name]
                for gt_box in gt_boxes:
                    c = np.array((0, 175, 0)) / 255.0
                    gt_box.render(ax, view=np.eye(4), colors=(c, c, c))
            ...
            data_path, _, camera_intrinsic = self.nusc.get_sample_data(sample_data_token,
