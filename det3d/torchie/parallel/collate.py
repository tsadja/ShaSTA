...
def collate_kitti(batch_list, samples_per_gpu=1):
    ...
    for example in batch_list:
        if type(example) is list:
            ...
            (data, data_prev) = example
            for k, v in data.items():
                example_merged[k].append(v)
            for k, v in data_prev.items():
                new_k = "prev_" + k
                example_merged[new_k].append(v)
        else:
    ...
    for key, elems in example_merged.items():
        if key in ["voxels", "num_points", "num_gt", "voxel_labels", "num_voxels",
                   ...
                   "prev_voxels", "prev_num_points", "prev_num_voxels", 
                    ]:
            ret[key] = torch.tensor(np.concatenate(elems, axis=0))
        elif key in ["det_boxes", "prev_det_boxes", "det_boxes_emb", "prev_det_boxes_emb", "gt", "gt_matched", "gt_newborn",
                    ]:
            ...
            ret[key] = res
        elif key in ["metadata", "prev_metadata"]:
            ret[key] = elems
        elif key == "calib":
            ret[key] = {}
            for elem in elems:
        ...
        elif key in ["points", "prev_points"]:
            ret[key] = [torch.tensor(elem) for elem in elems]
        elif key in ["prev_coordinates", "coordinates", "cyv_coordinates"]:
            coors = []
            ...
            ret[key] = torch.tensor(np.stack(elems, axis=0))
        elif key in ["cls_det_boxes", "prev_cls_det_boxes"]:
            ret[key] = elems
        else:
