...
def example_to_device(example, device=None, non_blocking=False) -> dict:
    ...
    for k, v in example.items():
        if k in ["anchors", "anchors_mask", "reg_targets", "reg_weights", "labels", 'points', 'prev_points']:
            ...
            example_torch[k] = v.cuda().to(dtype=torch.float)
            # example_torch[k] = v.to(device=device, non_blocking=non_blocking, dtype=torch.float)
        elif k == "calib":
            calib = {}
            for k1, v1 in v.items():
                # calib[k1] = torch.tensor(v1, dtype=dtype, device=device)
                calib[k1] = torch.tensor(v1).cuda()
                # calib[k1] = torch.tensor(v1).to(device, non_blocking=non_blocking)
            example_torch[k] = calib
        elif k in ["num_det_boxes", "num_prev_det_boxes"]:
            example_torch[k] = torch.tensor(v).cuda().to(dtype=torch.float)
        else:
            example_torch[k] = v
...
def track_batch_processor(model, data, train_mode=True, **kwargs):
    ...
    out = model(example, train_mode=train_mode)
    del example
