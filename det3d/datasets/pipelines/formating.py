...
class Reformat(object):
    ...
    def __call__(self, res, info):
        ...
        if 'det_boxes' in info.keys():
            data_bundle.update(det_boxes=info["det_boxes"])

        if 'cls_det_boxes' in info.keys():
            data_bundle.update(cls_det_boxes=info['cls_det_boxes'])

        if 'prev_cls_det_boxes' in info.keys():
            data_bundle.update(prev_cls_det_boxes=info['prev_cls_det_boxes'])

        if 'num_prev_det_boxes' in info.keys():
            data_bundle.update(num_prev_det_boxes=info['num_prev_det_boxes'])

        if 'num_det_boxes' in info.keys():
            data_bundle.update(num_det_boxes=info['num_det_boxes'])

        if 'prev_det_boxes' in info.keys():
            data_bundle.update(prev_det_boxes=info["prev_det_boxes"])

        if 'gt' in info.keys():
            data_bundle.update(gt=info["gt"])

        if 'gt_matched' in info.keys():
            data_bundle.update(gt_matched=info["gt_matched"])

        if 'gt_newborn' in info.keys():
            data_bundle.update(gt_newborn=info["gt_newborn"])
        
        if "targets" in res["lidar"]:
            if 'gt_boxes_and_cls' in res["lidar"]["targets"].keys():
                data_bundle.update(gt_boxes_and_cls = res["lidar"]["targets"]["gt_boxes_and_cls"])
